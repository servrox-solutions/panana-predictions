    ...
    <div className="grid flex-1 items-start gap-4 p-4 sm:px-6 sm:py-0 md:gap-8 lg:grid-cols-3 xl:grid-cols-3">
      <div className="grid auto-rows-max items-start gap-4 md:gap-8 lg:col-span-3">
        <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-2 xl:grid-cols-4">
          <Card className="col-span-2 lg:col-span-1" x-chunk="dashboard-05-chunk-2">
                ...
                <div className="flex gap-6 flex-wrap">
                  {
                    Object.entries(openMarkets).map(([asset, amount]) => (
                      <div className="flex items-center gap-1">{amount} <Web3Icon className="w-8 h-8" asset={asset as SupportedAsset} /></div>
                    ))
                  }
                </div>
              </CardTitle>
            ...
            <CardContent>
              <div className="text-xs text-muted-foreground">Total: {Object.values(openMarkets).reduce((prev, cur) => prev + cur, 0)}</div>
            </CardContent>
          </Card>
          <Card className="col-span-2 lg:col-span-1" x-chunk="dashboard-05-chunk-1">
            <CardHeader className="pb-2">
          ...
          <Card className="col-span-2 hidden md:block lg:col-span-1" x-chunk="dashboard-05-chunk-1">
            <CardHeader className="pb-2">
              <CardDescription>Volume last 10 Markets</CardDescription>
              <CardTitle className="text-4xl">
                $ {latestResolvedMarkets.slice(0, 10).reduce((prev, cur) => prev += cur.marketCap.usd, 0)}
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-xs text-muted-foreground">
                ({latestResolvedMarkets.slice(0, 10).reduce((prev, cur) => prev += cur.marketCap.asset, 0)} APT)
              </div>
            </CardContent>
          </Card>
          <Card className="col-span-2 hidden md:block lg:col-span-1" x-chunk="dashboard-05-chunk-1">
            <CardHeader className="pb-2">
              ...
              <CardTitle className="text-4xl">
                {latestResolvedMarkets.filter(x => +x.endTimeTimestamp >= DateTime.now().minus({ hours: 1 }).toSeconds()).length}
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-xs text-muted-foreground">
                ({latestResolvedMarkets.filter(x => +x.endTimeTimestamp >= DateTime.now().minus({ hours: 1 }).toSeconds()).reduce((prev, cur) => prev += cur.marketCap.asset, 0)} APT)
              </div>
            </CardContent>
          </Card>
        </div>
