...
export const ResolveMarketButton = (props: ResolveMarketButtonProps) => {
    ...
    const [resolveMarketInSeconds, setResolveMarketInSeconds] = useState(Math.floor(marketData.endTime - DateTime.now().toSeconds()));
    ...
    return (
        ...
        >
            <span>Resolve Market {isMounted && <>in {Duration.fromObject({ seconds: resolveMarketInSeconds }).toFormat('hh:mm:ss')} </>}</span>
        </LoadingButton >
    );
}