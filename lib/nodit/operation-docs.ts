...
export const ProfilePageStatsQuery = (sender: Address, moduleAddress: Address) => `
  query StatisticsPageQuery {
        created_markets: account_transactions_aggregate(
            where: {account_address: {_eq: "${sender}"}, user_transaction: {entry_function_id_str: {_eq: "${moduleAddress}::market::create_market"}}}
        ) {
            aggregate {
            count
            }
        ...
        total_votes: account_transactions_aggregate(
            where: {account_address: {_eq: "${sender}"}, user_transaction: {entry_function_id_str: {_eq: "${moduleAddress}::market::vote"}}}
        ) {
            aggregate {
            count
            }
        ...
        placed_bets: coin_activities_aggregate(
            where: {owner_address: {_eq: "${sender}"}, entry_function_id_str: {_eq: "${moduleAddress}::market::place_bet"}, activity_type: {_eq: "0x1::coin::WithdrawEvent"}}
        ) {
            aggregate {
                count
            }
