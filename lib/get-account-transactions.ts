import { CommittedTransactionResponse, TransactionResponse } from '@aptos-labs/ts-sdk';
...
export const getUnlimitedAccountTransactions = async (
  ...
  const promises = new Array(Math.ceil(amount / maxLimit)).fill(0).map((_, idx) => {
...
export const getTotalTransactionCount = (accountAddress: string) => {
  return aptos.getAccountTransactionsCount({ accountAddress });
}
