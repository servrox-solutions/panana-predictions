...
function scheduleCreateMarket(marketplace: AvailableMarketplace, min: number) {
  ...
  scheduleJob(rule, async () => {
    await createMarket(marketplace);
  });
  console.log(
...
async function createMarket(marketplace: AvailableMarketplace) {
  try {
...
function scheduleExecutionWithRetry(
  ...
  const job = scheduleJob(jobName, start.toJSDate(), async (fireDate) => {
    try {
      ...
      if (!res.success) {
        retryExecution(job, jobName, fireDate, numberRetries++, retryCount);
      } else {
...
function retryExecution(
  ...
  const retryDelay = (numberRetries + 1) * 3;
  console.error(
