...
export default async function Dashboard() {
  ...
  const createdEvents = await Promise.all(marketplaces.map(marketplace => getMarketCreatedEvents(MODULE_ADDRESS_FROM_ABI, marketplace.typeArgument)));
  const resolvedEvents = await Promise.all(marketplaces.map(marketplace => getMarketResolvedEvents(MODULE_ADDRESS_FROM_ABI, marketplace.typeArgument)));
  const price = await fetchPriceUSD('aptos');
  const allMarketplaces = await Promise.all(marketplaces.map(marketplace => getMarketplaceRessource({ address: marketplace.address, type: marketplace.typeArgument })));
  const totalVolumeApt = allMarketplaces.map(marketplace => marketplace.all_time_volume).reduce((prev, cur) => prev + cur, 0);
  const totalVolume = {
  ...
  // TODO: sort data
  const createdMarkets = createdEvents.map((marketplaceEvents, idx) => marketplaceEvents.map(x => ({
    creator: x.creator,
    ...
    minBet: +x.min_bet,
  }))).flat().sort((x, y) => y.createdAtTimestamp - x.createdAtTimestamp);

  const resolvedMarkets = resolvedEvents.map((marketplaceEvents, idx) => marketplaceEvents.map(x => ({
    endTimeTimestamp: +x.end_time_timestamp,
    assetSymbol: symbols[idx],
    startTimeTimestamp: +x.start_time_timestamp,
    marketAddress: x.market.inner,
  ...
  }))).flat().sort((x, y) => y.endTimeTimestamp - x.endTimeTimestamp);

  return (
    <>
      <DashboardContent totalVolume={totalVolume} latestCreatedMarkets={createdMarkets} latestResolvedMarkets={resolvedMarkets} />
      <ToastContainer />
    </>
  );
}